...
final echoAction = Action<String, String>(
  ...
  runner: (input) => input,
    ...
    .onlyForPermissions(); // Not implemented yet, but could limit it to users with certain permissions
```
...
The Action module is already integrated with the Flood framework through the `FloodCoreComponent`, so you can start using it right away in your Flood applications.
