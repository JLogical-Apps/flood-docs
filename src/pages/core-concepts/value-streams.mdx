...
1. `mapWithValue`: This method is similar to the standard `map` operator, but it allows you to retrieve the current value of the transformed stream synchronously.
...
2. `switchMapWithValue`: This method is equivalent to the switchMap operator, but it maintains the ability to retrieve the current value of the resulting stream synchronously.
...
3. `asyncMapWithValue`: This method is similar to the asyncMap operator, but it allows you to retrieve the latest value emitted by the stream. However, it's important to note that the async mapper only works when the stream is being listened to.
    ```dart
...
-   `combineLatestWithValue`: This method is similar to the `combineLatest` operator from `rxdart`, but it allows you to retrieve the current combined value synchronously.
